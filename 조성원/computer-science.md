...
### 4. HTTP/1.0 과 HTTP/1.1의 차이는 뭔가요?
...
### 5. HTTP/1.1 과 HTTP/2.0의 차이는 뭔가요?
...
### 6. 프로세스는 어떤 구조로 이루어져 있나요? (스택, 힙, 데이터 영역, 코드 영역)
...
### 7. PCB는 무엇인가요? 어떤 용도로 사용되나요? (메타 데이터)
...
### 6. JWT를 웹 스토리지에 저장해야 한다면 어디다 저장하시겠나요? 이유는요?
...
- 코드의 주요 흐름들을 통합적으로 테스트
- 주요 외부 의존성(DB 등)에 대한 테스트
...
- 최종 사용자의 흐름에 대한 테스트
- 외부로부터의 요청부터 응답까지 기능이 잘 동작하는지에 대한 테스트
...
- 웹팩에서의 모듈
  ...
  - 웹 애플리케이션을 제작하려면 HTML, CSS, JavaScript, Images, Font 등 많은 파일들이 필요 → 각 파일이 모두 모듈

&nbsp;

### 3. 모듈 번들링이란?

![모듈 번들링](./images/bundling.png)
...
1. 파일 단위의 자바스크립트 모듈 관리의 필요성
   - 자바스크립트 변수(`var`)는 기본적으로 전역 스코프이기 때문에 중복 선언하거나 의도치 않은 할당이 발생
   - 파일 단위로 변수를 관리하고 싶은 욕구
2. 웹 개발 작업 자동화 도구
   - 이전부터 프런트엔드 개발 업무를 할 때 가장 많이 하는 작업은 코드 편집 후 새로 고침
   - 이 외에도 HTML/CSS/JS 압축, 이미지 압축, CSS 전처리 등의 작업을 해야 했다.
3. 웹 애플리케이션의 빠른 로딩 속도와 높은 성능
   - 웹 사이트의 로딩 속도를 높이기 위해 **브라우저에서 서버로 요청하는 파일 수 ↓**
   - 초기 페이지 로딩 속도를 높이기 위해 나중에 필요한 자원을 나중에 요청하는 **Lazy Loading**이 등장
     - 웹팩의 기본 철학

#### 웹팩 사용 시에 이점

- 자바스크립트 변수 유효 범위
  - ES6 Modules와 웹팩의 모듈 번들링으로 해결
- 브라우저별 HTTP 요청 숫자의 제약
  - 웹팩으로 여러 개의 파일을 하나로 합치면 브라우저별 HTTP 요청 숫자 제약을 피할 수 있다.
- 사용하지 않는 코드의 관리
...
- 최신 자바스크립트 문법으로 작성된 코드를 이전 자바스크립트 문법으로 변환해주는 도구(트랜스파일러)
- ES6에서 새로운 기능들이 많이 추가되었는데, 예전 브라우저에서는 호환이 되지 않는 문제가 발생
...
- Entry
  - 번들링을 시작할 파일을 지정하는 속성
  - entry 속성에 명시된 파일을 기준으로 의존성 트리를 만들어 하나의 번들 파일을 생성
- Output
  - 번들링된 파일의 이름과 경로를 지정하는 속성
- Loader
  - 웹팩은 기본적으로 자바스크립트 파일만 이해
  - CSS, 이미지 등의 파일을 웹팩이 이해할 수 있는 형태로 변환하는 작업을 수행
  - Babel Loader, Sass Loader, File Loader, Vue Loader, TS Loader 등
- Plugin
  - 추가적인 기능을 제공하는 플러그인을 지정

&nbsp;

## 타입과 인터페이스 🔥🔥🔥

### 1. 타입스크립트를 왜 쓰나요? (본인이 느낀점)

- `const`, `let`으로 변수를 선언하며 런타임에 타입이 지정되는데, 코드의 관점에서 직관적이지 않음
- 런타임에서 타입과 관련된 오류가 발생했을 때, 원인을 찾기가 힘듦
- 어떤 변수가 객체나 배열일 때, 세부 속성을 파악하기 힘듦
- 다른 사람과 협업할 때, 코드를 신뢰하기 어려움

> 타입스크립트를 사용하면 위의 문제점을 모두 해결할 수 있다.

&nbsp;

### 2. 타입과 인터페이스의 차이를 아나요?

- 객체의 구조를 정의하는 두 가지 주요 방법
- 변수, 함수 반환 값, 매개변수 등의 타입을 정의하는데 사용

- 인터페이스는 동일한 이름으로 여러 번 선언할 수 있으며, 이 선언들은 자동으로 병합된다.
  - 재사용성과 확장성이 좋으며, 타입스크립트 팀에서도 인터페이스 사용을 권장한다.
  - 클래스가 특정 구조를 따르도록 강제할 수 있으며, 이는 코드의 일관성을 유지하는 데 도움이 된다.
- 타입은 동일한 이름으로 여러 번 선언할 수 없으며, 병합되지 않는다.
  - 병합은 불가능하지만, 더 다양한 타입을 정의할 수 있다.
    - 유니언 타입, 튜플, 리터럴 타입 등을 정의할 수 있다.
  - 타입스크립트의 강력한 타입 시스템을 활용하는 데 도움이 된다.

&nbsp;

### 3. 타입의 유틸리티 타입 중 알고 있는 것들에 대해 가볍게 설명해주세요
